...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 w-full z-50 border-b border-orange-500/10 transition-all duration-300 ${
          isOpen
              ...
              {navLinks.map((link, index) => (
                <li key={index} className="relative group">
                  <a
                    ...
                    className={`transition duration-300 font-medium hover:text-orange-400 ${
                      active === link.name
                        ? "text-orange-400"
                        : "text-gray-200"
                    }`}
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden fixed top-20 left-0 w-full z-50 overflow-hidden transition-all duration-500 ease-out transform ${
      ...
      >
        <ul className="flex flex-col items-center gap-6 py-8 bg-[#111111]/95 backdrop-blur-xl border-b border-orange-500/10">
          ...
          <button className="bg-orange-500 flex items-center justify-center gap-2 hover:bg-orange-600 active:scale-95 px-10 py-2.5 rounded-lg font-semibold transition duration-300">
            <Download size={16} />
            Resume
          </button>
